...
const PORTAL_DRAFT_TAG = "nct-reseller-portal";
const PORTAL_DRAFT_TIMEOUT_MS = 5 * 60 * 1000;
...
function getDraftExpiryTimestamp(createdAt) {
  const createdAtMs = new Date(createdAt || 0).getTime();
  if (!Number.isFinite(createdAtMs) || createdAtMs <= 0) return null;
  return new Date(createdAtMs + PORTAL_DRAFT_TIMEOUT_MS).toISOString();
}

function getRemainingDraftMs(createdAt) {
  const expiresAt = getDraftExpiryTimestamp(createdAt);
  if (!expiresAt) return 0;
  return Math.max(0, new Date(expiresAt).getTime() - Date.now());
}

function isPortalDraftOrder(order) {
  return normalizeTags(order?.tags).includes(PORTAL_DRAFT_TAG);
}

function isExpiredPortalDraft(order) {
  return getRemainingDraftMs(order?.created_at) <= 0;
}

function cloneProductWithReservations(product) {
  return {
    ...product,
...
function applyPortalDraftReservations(products, draftOrders) {
  const reservedByVariant = new Map();

  for (const order of draftOrders || []) {
    for (const item of order.items || []) {
      if (!item.variantLegacyId) continue;
      const current = reservedByVariant.get(item.variantLegacyId) || 0;
      reservedByVariant.set(item.variantLegacyId, current + Number(item.quantity || 0));
    }
  }

  return (products || []).map((product) => {
    const nextProduct = cloneProductWithReservations(product);
    let totalInventory = 0;

    nextProduct.variants = nextProduct.variants.map((variant) => {
      const reserved = reservedByVariant.get(String(variant.legacyId || "")) || 0;
      const baseInventory = Number(variant.inventory || 0);
      const inventory = Math.max(0, baseInventory - reserved);
      totalInventory += inventory;

      return {
        ...variant,
        inventory,
        availableForSale: variant.inventory == null ? variant.availableForSale : inventory > 0,
    ...
    return {
      ...
      availableForSale: nextProduct.variants.some((variant) => variant.availableForSale),
    };
  });
}

export function getPortalDraftTimeoutMs() {
  return PORTAL_DRAFT_TIMEOUT_MS;
}
...
export function getShopifyStoreUrl() {
  return getShopifyPublicStoreUrl();
}
...
export async function deleteShopifyDraftOrder(draftOrderId) {
  if (!draftOrderId) {
    throw new Error("A draft order id is required to delete a Shopify draft order.");
  }

  await shopifyAdminRequest(`/draft_orders/${draftOrderId}.json`, {
    method: "DELETE",
  });
...
export async function fetchActivePortalDraftOrders({ email, limit = 250, cleanupExpired = true } = {}) {
  const payload = await shopifyAdminRequest(`/draft_orders.json?status=open&limit=${limit}&fields=id,name,email,invoice_url,created_at,status,total_price,currency,line_items,tags`);
  const normalizedEmail = String(email || "").toLowerCase();
  const portalDrafts = (payload?.draft_orders || []).filter((order) => isPortalDraftOrder(order));
  const expiredDrafts = portalDrafts.filter((order) => isExpiredPortalDraft(order));

  if (cleanupExpired && expiredDrafts.length > 0) {
    await Promise.allSettled(
      expiredDrafts.map((order) =>
        deleteShopifyDraftOrder(order.id).catch((error) => {
          console.error("Shopify draft order cleanup error:", error);
        })
      )
    );
  }
  ...
  return portalDrafts
    .filter((order) => !isExpiredPortalDraft(order))
    .filter((order) => !normalizedEmail || String(order.email || "").toLowerCase() === normalizedEmail)
    .map((order) => mapShopifyDraftOrder(order));
}

export async function fetchReservationAwareShopifyCatalog(options = {}) {
  const catalog = await fetchShopifyCatalog(options);

  if (catalog.source !== "shopify" || !catalog.configured) {
    return catalog;
  }

  const activeDrafts = await fetchActivePortalDraftOrders();
  const products = applyPortalDraftReservations(catalog.products || [], activeDrafts);

  return {
    ...catalog,
    products,
    reservation_timeout_ms: PORTAL_DRAFT_TIMEOUT_MS,
    active_reserved_checkouts: activeDrafts.length,
  };
}

export async function fetchShopifyDraftOrderById(draftOrderId) {
  if (!draftOrderId) return null;

  const payload = await shopifyAdminRequest(`/draft_orders/${draftOrderId}.json?fields=id,name,email,invoice_url,created_at,status,total_price,currency,line_items,tags`);
  const draftOrder = payload?.draft_order || null;

  if (!draftOrder || !isPortalDraftOrder(draftOrder)) {
    return null;
  }

  if (draftOrder.status === "open" && isExpiredPortalDraft(draftOrder)) {
    await deleteShopifyDraftOrder(draftOrder.id);
    return null;
  }

  return mapShopifyDraftOrder(draftOrder);
}
  ...
  const payload = await shopifyAdminRequest("/draft_orders.json", {
    ...
    body: {
      draft_order: {
        ...
        note: note || null,
        tags: ["nct-reseller-portal", reseller.tier || "public"].filter(Boolean).join(", "),
...
function mapShopifyDraftOrder(order) {
  return {
    ...
    source: "draft",
    expiresAt: getDraftExpiryTimestamp(order.created_at),
    timeoutMs: PORTAL_DRAFT_TIMEOUT_MS,
    items: (order.line_items || []).map((line) => ({
      id: line.id ? String(line.id) : `${order.id}-${line.title}`,
      title: line.title,
      quantity: Number(line.quantity || 0),
      ...
      sku: line.sku || null,
      variantTitle: line.variant_title || null,
      variantLegacyId: line.variant_id ? String(line.variant_id) : null,
...
export async function fetchShopifyOrderHistoryByEmail({ email, limit = 20 }) {
  const [ordersPayload, draftsPayload] = await Promise.all([
    ...
    shopifyAdminRequest(`/draft_orders.json?status=open&limit=${limit}&fields=id,name,email,invoice_url,created_at,status,total_price,currency,line_items,tags`),
  ]);
  ...
  const expiredDrafts = (draftsPayload?.draft_orders || [])
    .filter((order) => isPortalDraftOrder(order))
    .filter((order) => isExpiredPortalDraft(order));
  ...
  if (expiredDrafts.length > 0) {
    await Promise.allSettled(
      expiredDrafts.map((order) =>
        deleteShopifyDraftOrder(order.id).catch((error) => {
          console.error("Shopify draft order expiry cleanup error:", error);
        })
      )
    );
  }
