...
async function getAccountMaps(db, enrollmentIds) {
  ...
  const enrollmentRows = await runOptional(
    db
  ...
  const orgIds = (enrollmentRows || []).map((row) => row.organization_id).filter(Boolean);

  const [inventoryRows, requestRows, legacyRows, organizationRows] = await Promise.all([
    runOptional(
      db
