...
export async function createCanonicalDiscardPickup(db, legacyPickupId, legacyAccountId, payload) {
  ...
  const stopNotes = [
    ...
    payload.contamination_reported ? `Contamination (${payload.contamination_severity || "flagged"}): ${normalizeNullableString(payload.contamination_notes) || "see admin notes"}` : null,
  ].filter(Boolean).join("\n") || null;
  ...
  const payoutResult = await runOptional(
    db.from("pickup_payouts").insert({
      ...
      calculated_from: {
        ...
        legacy_contamination_reported: !!payload.contamination_reported,
        legacy_contamination_severity: payload.contamination_severity || null,
        legacy_contamination_source: payload.contamination_source || null,
        legacy_contamination_notes: payload.contamination_notes || null,
      },
      notes: runNotes,
    })
  );
